...
export default {
  ...
  methods: {
    ...
    tanchuang() {
      this.$alert(`<div style="text-align:center;font-size:15px"><strong><span style="font-size:20px;color:red">apiurl.v1.mk已被墙，请更换最新的url.v1.mk</span></strong></br><strong><span style="font-size:20px">简易版：</span><span><a href="https://sub.yfjc.my" target="_blank" style="color:red;font-size:20px;text-decoration:none">点击跳转</a></span></strong></br><strong><span style="font-size:20px">简单易用</br></div>`, '信息面板', {
    ...
    getBackendVersion() {
      this.$axios
        ...
        .then(res => {
          ...
          a ? this.$message.success("肥羊增强版后端，已屏蔽免费节点池（会返回403），额外支持vless reality+hysteria+hysteria2订阅转换，项目搭建来自肥羊和CMliussss") : b ? this.$message.success(`${this.backendVersion}` + "本地局域网自建版后端") : this.$message.success(`${this.backendVersion}`);
        })
        .catch(() => {
